...
package uk.gov.hmrc.outcomeauditproxy.controllers
...
import uk.gov.hmrc.outcomeauditproxy.config.AppConfig
...
class OutcomeAuditingControllerSpec extends AnyWordSpec with Matchers with GuiceOneAppPerSuite {
  ...
  override lazy val app: Application = new GuiceApplicationBuilder()
    ...
    .configure("microservice.services.outcome-audit.authToken" -> authToken)
    ...
    "forward a 200 response from the downstream service" in {
        ...
        import components.{defaultActionBuilder => Action}
      {
        case r@SPOST(u) if u.path == url =>
          ...
          r.headers.get("Authorization") shouldBe Some("Basic " + AppConfig.createAuth("outcome-audit-proxy", authToken))
