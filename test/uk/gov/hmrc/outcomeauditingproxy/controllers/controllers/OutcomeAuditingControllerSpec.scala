...
package uk.gov.hmrc.outcomeauditingproxy.controllers.controllers
...
import uk.gov.hmrc.outcomeauditingproxy.config.AppConfig
import uk.gov.hmrc.outcomeauditingproxy.controllers.OutcomeAuditingController

import scala.concurrent.Future
import scala.concurrent.duration.DurationInt
import scala.language.postfixOps

class OutcomeAuditingControllerSpec extends AnyWordSpec with Matchers with GuiceOneAppPerSuite {
  ...
  override lazy val app: Application = new GuiceApplicationBuilder()
    .configure("microservice.services.outcome-auditing.port" -> outcomePort)
    .configure("microservice.services.outcome-auditing.authToken" -> authToken)
  ...
  private val controller = app.injector.instanceOf[OutcomeAuditingController]
  implicit val mat: Materializer = app.injector.instanceOf[Materializer]

  "POST /report/outcome" when {
    "the user-agent is on the allow list" should {
      ...
      behave like downstreamConnectorEndpoint("/report/outcome", response) { () =>
        controller.reportOutcome()(
          FakeRequest("POST", "/report/outcome").withJsonBody(Json.parse(request)).withHeaders(headers: _*))
      }
    }
    ...
    "the user-agent is NOT on the allow list" should {
      "return forbidden" in {
        ...
        val result = controller.reportOutcome()(
          FakeRequest("POST", "/report/outcome").withJsonBody(Json.parse(request)).withHeaders(headers: _*))
    ...
    "forward a 200 response from the downstream service" in {
        ...
        import components.{defaultActionBuilder => Action}
      {
        case r@SPOST(u) if u.path == url =>
          ...
          r.headers.get("Authorization") shouldBe Some("Basic " + AppConfig.createAuth("outcome-auditing-proxy", authToken))
