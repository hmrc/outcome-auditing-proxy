...
import uk.gov.hmrc.outcomeauditingproxy.config.ExtendedServicesConfig
import uk.gov.hmrc.outcomeauditingproxy.controllers.AccessChecker.{accessControlAllowListAbsoluteKey, accessControlAllowListKey, accessControlEnabledKey, accessRequestFormUrlKey}
...
class AccessChecker @Inject()(config: Configuration) extends ExtendedServicesConfig(config) {
  ...
  def getClientFromUserAgent[T](req: Request[T]): Option[String] = {
    ...
    req.headers.getAll(HeaderNames.USER_AGENT).flatMap(_.split(","))
      .find(ua => ua != "outcome-auditing-proxy")
  }
}
